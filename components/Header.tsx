...
export function Header({
  ...
  return (
    <header className="sticky top-0 z-30 h-16 bg-white/90 backdrop-blur-xl border-b border-slate-200/80 px-3 sm:px-6 flex items-center justify-between shrink-0 shadow-xs print:hidden">
      ...
      <div className="flex items-center gap-2.5 min-w-0">
        ...
        <div className="sm:hidden font-bold text-sm text-slate-900 truncate">
          {currentBreadcrumb?.label || "FinLedger"}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        ...
        {/* Financial Year Tag */}
        <div className="flex items-center gap-1.5 bg-emerald-500/10 text-emerald-800 border border-emerald-500/20 px-2.5 sm:px-3 py-1 rounded-xl text-[11px] sm:text-xs font-bold font-tabular">
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
          <span>{activeFy}</span>
        </div>

        {/* Database Status */}
        <div className="hidden xs:flex items-center gap-1.5 bg-slate-100 border border-slate-200 px-2.5 py-1 rounded-xl text-[11px] font-bold text-slate-600">
